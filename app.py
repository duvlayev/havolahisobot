...
def check_duplicate(conn, url, tags, order_num):
    ...
    URL bo'yicha oldingi yozuvni qidiradi.

    Qaytadi:
      ('none', None)               — bunday link ilgari yaratilmagan
      ('exact', row)                — xuddi shu joyda (teglar) VA xuddi shu
                                       raqamda ilgari yaratilgan — rad etiladi
      ('different_context', row)    — boshqa joyda yoki boshqa raqamda ilgari
                                       yaratilgan — HA/YO'Q so'raladi
    """
    rows = conn.execute(
    ...
    for row in rows:
        if row['tags_json'] == tags_norm and row['order_num'] == order_num:
            return ('exact', row)

    # Xuddi shu URL, lekin boshqa kontekst (teg yo'li yoki raqam) — eng
    # so'nggisini namuna sifatida ko'rsatamiz
    return ('different_context', rows[0])
...
def process_single_link(conn, root_path, domains, chat_id, message_id, tags, order_num, url):
    ...
    if dup_kind == 'exact':
        note = f"Bu havola {' -> '.join('#'+t for t in tags)} ichida allaqachon papka yaratilgan. Papka nomi: {dup_row['folder_name']}"
        return {
            'action': 'rejected_duplicate',
    ...
    if dup_kind == 'different_context':
        ...
        note = (
            f"Bu havola {' -> '.join('#'+t for t in old_tags)} ichida allaqachon papka yaratilgan. "
...
def last_folder_path():
    board = get_board(request.args)
    return jsonify({'folder_path': get_setting(f'last_folder_path_{board}', '') or ''})
...
def papkaoch_list_entries():
    ...
    with get_db() as conn:
        rows = conn.execute(
            'SELECT * FROM papkaoch_entries ORDER BY id DESC LIMIT ?', (limit,)
        ).fetchall()
    items = []
    for r in rows:
        d = dict(r)
...
def papkaoch_logs():
    ...
    content = papkaoch_read_log(date_str) if date_str else ''
    ...
    with get_db() as conn:
        for u in updates:
            ...
            if row:
                ...
                if is_th:
                    conn.execute(
                        'INSERT INTO links (url, sana, likes, status, board, order_num, ajratilgan_izohlar, umumiy_izohlar) '
                        'VALUES (?, ?, 0, ?, ?, ?, ?, ?)',
                        (url, sana, row_status, board, order_num, count, count2)
                    )
                else:
                    conn.execute(
                        'INSERT INTO links (url, sana, likes, status, board, order_num) '
                        'VALUES (?, ?, ?, ?, ?, ?)',
                        (url, sana, count, row_status, board, order_num)
                    )
                added += 1
        conn.commit()
...
def add_link():
    ...
    with get_db() as conn:
        ...
        cur = conn.execute(
            'INSERT INTO links (url, sana, likes, status, board, umumiy_izohlar, ajratilgan_izohlar, order_num) '
            'VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            (url, sana, likes, status, board, umumiy_izohlar, ajratilgan_izohlar, order_num)
        )
        conn.commit()
