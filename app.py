...
def _discover_t_hisobot_category_dirs(root_path, path_prefix=None, max_depth=8):
    ...
    MUHIM: bitta papka ICHIDA bir vaqtning o'zida HAM raqamlangan post-
    papkalar (masalan "#Коммент" tegi YOLG'IZ — pastki rukun tegisiz —
    ishlatilganda to'g'ridan-to'g'ri "Коммент" ichiga ochilgan papkalar),
    HAM nomli rukun-pastki-papka (masalan "#Коммент #Ташриф" bilan
    ochilgan "Ташриф" papkasi) bo'lishi MUMKIN — bular BIR-BIRINI istisno
    qilmaydi. Shuning uchun bu yerda ikkalasi ham alohida-alohida
    tekshiriladi: agar shu darajaning o'zida raqamlangan papkalar bo'lsa —
    bu daraja ("Umumiy" yoki path_prefix nomi bilan) alohida rukun sifatida
    qo'shiladi, VA shu bilan bir qatorda — bu safar raqamlanmagan nomli
    pastki papkalar ham (bo'lishi mumkin bo'lgan yana bir "Ташриф" kabi
    rukunlarni topish uchun) alohida-alohida ICHIGA tushib tekshiriladi.
    ...
    # bormi? Bo'lsa — bu daraja ("barg") ALOHIDA rukun sifatida qo'shiladi.
    numbered_names = {name for name, _ in subdirs if _extract_link_folder_order(name) is not None}
    # Raqamlanmagan (nomli) pastki papkalar — bularning HAR biri o'zi
    # yana chuqurroqda raqamlangan papkalarga (yoki yana ichki rukunlarga)
    # ega bo'lishi mumkin, shu sabab bularga ALOHIDA-ALOHIDA tushib
    # tekshiramiz — hatto shu darajaning o'zida ham raqamlangan papkalar
    # (numbered_names) bo'lsa-da.
    named_subdirs = [(name, full) for name, full in subdirs if name not in numbered_names]

    results = []
    if numbered_names:
        label = ' / '.join(path_prefix) if path_prefix else 'Umumiy'
        results.append((label, root_path))

    for name, full in named_subdirs:
...
# Qator BOSHIDAGI raqam + (ixtiyoriy) ".", ")" yoki bo'shliq(lar) — bu qismdan
# KEYIN qolgan matnda (istalgan joyida — havoladan oldin izoh bo'lsa ham)
# havola qidiriladi. MUHIM: bu yerda qatorning OXIRI cheklanmagan ($ yo'q) —
# ya'ni havoladan KEYIN yana matn (izoh, "10 tadan", "Menga yoqdi" kabi
# har qanday erkin matn — postni yozgan xodim yozib qo'yishi mumkin bo'lgan
# har qanday narsa) bo'lsa ham, bu e'tiborsiz qoldiriladi va baribir link
# to'g'ri o'qiladi.
_LEADING_NUMBER_RE = re.compile(r'^\s*(\d+)\s*[.\)]?\s*')
# Qatorda FAQAT raqamning o'zi (ixtiyoriy "." yoki ")" bilan, boshqa hech
# narsa yo'q) — havola esa KEYINGI qatorda bo'lgan holat uchun ("1" / "1." /
# "1)" alohida qatorda).
_BARE_NUMBER_LINE_RE = re.compile(r'^\s*(\d+)\s*[.\)]?\s*$')

def parse_telegram_message(text):
    """Telegram xabar matnini tahlil qiladi.

    Kutilgan format — raqam va havola orasidagi belgi ixtiyoriy (nuqta,
    qavs, bir yoki bir nechta bo'shliq, hatto ayirmasdan yozilgan bo'lsa
    ham), raqam alohida qatorda bo'lsa ham, VA MUHIMI — havoladan KEYIN
    (yoki undan oldin, agar raqamdan keyin) har qanday qo'shimcha matn/izoh
    bo'lsa ham ("10 tadan", "Menga yoqdi", "bu link Eshmatga" va h.k. — postni
    yozgan xodim yozib qo'yishi mumkin bo'lgan ISTALGAN matn) — havola va
    uning tartib raqami baribir to'g'ri tushiniladi. Masalan quyidagilarning
    barchasi bir xil natija beradi ("3-raqamli papka, shu havola bilan"):
        3. https://... 10 tadan
        3 https://...  bu link Eshmatga
        3)      https://... 100
        3.https://...
        3
        https://... 10 ta

    QOIDA: har bir qatorda link OLDIDAN birinchi uchragan (o'sha qatorning
    boshidagi yoki undan oldingi alohida qatordagi) raqam — o'sha link
    (papka)ning tartib raqami hisoblanadi.

    MUHIM: agar biror qatorda HAVOLA bor-u, lekin unga mos TARTIB RAQAMI
    (na o'sha qatorning boshida, na undan oldingi alohida qatorda)
    topilmasa — bu XATOLIK hisoblanadi ("Havola raqamlanmagan") va shu
    havola uchun papka OCHILMAYDI — chaqiruvchi tomon (papkaoch_handle_
    message) bu haqda guruhga ochiq xabar yuboradi (tovushsiz e'tiborsiz
    qoldirilmaydi).

    Qaytadi: {'tags': [...], 'links': [(raqam, url), ...], 'unnumbered': [url, ...]}
    — agar xabarda KAMIDA bitta teg VA (kamida bitta raqamlangan havola
    YOKI kamida bitta raqamsiz/xato havola) bo'lsa. Aks holda — None (bu
    xabar botga umuman tegishli emas — # yoki havola yo'q)."""
    if not text:
        return None
    ...
    for i, line in enumerate(raw_lines):
        ...
        # 1) Qator RAQAM bilan boshlanadimi? ("3.", "3)", "3", "3   " va h.k.)
        lm = _LEADING_NUMBER_RE.match(line)
        if lm:
            rest = line[lm.end():]
            # Havola raqamdan keyin — bu qatorning ISTALGAN joyida bo'lishi
            # mumkin (oldin izoh bo'lishi kamdan-kam, lekin ehtiyot uchun
            # butun qolgan matndan qidiramiz, faqat BIRINCHI uchraganini olamiz).
            url_match = _URL_RE.search(rest)
            if url_match:
                links.append((int(lm.group(1)), url_match.group(0)))
                continue
            # Raqam bilan boshlangan, lekin bu qatorda havola umuman yo'q
            # (masalan yolg'iz "3" — havola KEYINGI qatorda bo'lishi mumkin,
            # yoki "3. Izoh matni" — botga aloqasi yo'q) — xatolik hisoblanmaydi,
            # shunchaki e'tiborsiz qoldiriladi (pastdagi qatorlar tekshiruvida
            # "oldingi qator" sifatida hisobga olinadi).
            continue

        # 2) Shu qatorning o'zida havola bor, lekin qator raqam bilan
        # boshlanmagan — oldingi (bo'sh bo'lmagan) qatorga qaraymiz: u
        # yolg'iz raqammi?
        ...
        while j >= 0 and not raw_lines[j].strip():
            j -= 1
        if j >= 0:
            bm = _BARE_NUMBER_LINE_RE.match(raw_lines[j])
            if bm:
                order_num = int(bm.group(1))

        if order_num is not None:
            links.append((order_num, url_match.group(0)))
        else:
            # Havola bor, lekin unga mos tartib raqami hech qayerda
            # topilmadi — xatolik sifatida qayd etamiz.
            unnumbered.append(url_match.group(0))

    if not links and not unnumbered:
        return None

    return {'tags': tags, 'links': links, 'unnumbered': unnumbered}
...
def _first_existing_folder_entry(conn, rows):
    """Berilgan (bir xil shart bo'yicha topilgan, id DESC tartibida) qatorlar
    orasidan papkasi DISKDA HALI HAM jismonan mavjud bo'lgan BIRINCHISINI
    qaytaradi.

    MUHIM (avtomatik tozalash): agar bazada "created" deb belgilangan biror
    yozuvning FIZIK papkasi diskdan (masalan hodim xato havolani angab,
    papkani qo'lda Windows Explorer orqali) allaqachon o'chirilgan bo'lsa —
    bu yozuv ESKIRGAN (stale) hisoblanadi va endi haqiqiy TAKRORIYLIK
    sifatida HISOBGA OLINMAYDI — bunday eskirgan yozuvlar shu yerda
    avtomatik ravishda bazadan ham tozalab tashlanadi. Bu — admin har safar
    "Papkaoch -> Oxirgi faoliyat" bo'limiga kirib qo'lda qidirib
    o'chirishga majbur bo'lmasligi uchun (garchi qo'lda o'chirish imkoniyati
    ham mavjud bo'lib qolaveradi — masalan disk hali tekshirib ulanmagan
    holatlar uchun)."""
    stale_ids = []
    found = None
    for row in rows:
    ...
    if stale_ids:
        ...
        conn.execute(f'DELETE FROM papkaoch_entries WHERE id IN ({placeholders})', stale_ids)
        conn.commit()
    return found
...
def check_duplicate(conn, url, tags, order_num):
    ...
    MUHIM: har ikkala tekshiruv ham FAQAT papkasi DISKDA HALI HAM mavjud
    bo'lgan yozuvlarni hisobga oladi (yuqoridagi _first_existing_folder_entry
    izohiga qarang) — bazada qolib ketgan, lekin papkasi allaqachon
    o'chirilgan "eskirgan" yozuvlar avtomatik e'tiborsiz qoldiriladi (va
    tozalanadi), takroriylik sifatida bloklamaydi.
    ...
    same_folder = _first_existing_folder_entry(conn, same_folder_rows)
    if same_folder:
        return ('same_folder_conflict', same_folder)

    other_folder_rows = conn.execute(
        "SELECT * FROM papkaoch_entries WHERE url=? AND status='created' ORDER BY id DESC",
        (url,)
    ).fetchall()
    other_folder = _first_existing_folder_entry(conn, other_folder_rows)
    if other_folder:
...
def process_single_link(conn, root_path, domains, chat_id, message_id, tags, order_num, url):
    ...
    # MUHIM: bazada bu havola/raqam bo'yicha eski yozuv yo'q (masalan admin
    # "Oxirgi faoliyat" bo'limidan xato yozuvni o'chirgan), LEKIN diskdagi
    # papka FIZIK jihatdan hali ham mavjud bo'lishi mumkin (masalan hodim
    # bazadan o'chirdi-yu, papkani diskdan o'chirishni unutdi, yoki eski va
    # yangi havola bir xil domen/raqamga ega bo'lgani uchun papka nomi
    # tasodifan bir xil chiqdi). Bunday holatda "yangi papka ochildi" deyish
    # noto'g'ri (chalg'ituvchi) bo'lardi — shu sabab avval papka diskda
    # ALLAQACHON mavjudligini tekshiramiz.
    folder_pre_existed = os.path.isdir(full_path)
    ...
    if ok:
        ...
        conn.commit()
        if folder_pre_existed:
            return {
                'action': 'created', 'folder_name': folder_name, 'full_path': full_path,
                'matched_existing': True,
                'note': f"Papka havolaga moslandi: {folder_name}",
            }
...
def papkaoch_handle_message(update):
    ...
    created = [(o, u, r) for o, u, r in results if r['action'] == 'created' and not r.get('matched_existing')]
    matched_existing = [(o, u, r) for o, u, r in results if r['action'] == 'created' and r.get('matched_existing')]
    ...
    unnumbered = parsed.get('unnumbered') or []
    ...
    if matched_existing:
        # MUHIM: bazada bu havola bo'yicha eski yozuv yo'q edi (masalan admin
        # xato yozuvni "Oxirgi faoliyat"dan o'chirgan edi), LEKIN diskda shu
        # nomdagi papka ALLAQACHON mavjud ekan (masalan eski papka qo'lda
        # o'chirilmagan, yoki yangi to'g'ri havola eskisi bilan bir xil
        # nom/raqamga tushib qoldi). Bu holat "yangi papka ochildi"dan farqli
        # o'laroq alohida ko'rsatiladi — chalkashlik bo'lmasligi uchun.
        if lines:
            lines.append('')
        lines.append(f"🔗 {len(matched_existing)} ta havola uchun mavjud papka havolaga moslandi:")
        for o, u, r in matched_existing:
    ...
    if unnumbered:
        # MUHIM: havola bor-u, lekin unga mos tartib raqami (na o'zi qatorda,
        # na undan oldingi alohida qatorda) topilmagan — bunday havola uchun
        # papka OCHILMAYDI, va bu jimgina o'tkazib yuborilmasligi kerak —
        # guruhga ochiq xabar bilan bildiriladi (foydalanuvchi qaysi
        # havola(lar) raqamlanmagani va nima uchun rad etilganini ko'rsin).
        if lines:
            lines.append('')
        lines.append(f"⚠️ {len(unnumbered)} ta havola raqamlanmagani uchun ochilmadi:")
        for u in unnumbered:
            lines.append(f"  • Havola raqamlanmagan: {u}")
...
_SANA_ORDER_SUFFIX_RE = re.compile(r'\s*#\s*(\d+)\s*$')

def format_sana(row):
    """'Sana' ustuni uchun matn (Excel/Word eksportida ishlatiladi):

    (d) ENG SO'NGGI sana ko'rsatiladi — agar qator TAHRIRLANGAN bo'lsa oxirgi
    tahrir sanasi, aks holda dastlabki kiritilgan sana (ikkalasini birga
    "kiritilgan / tahrirlangan" ko'rinishida ko'rsatish EMAS — veb-jadvalda
    aniqlik uchun ikkalasi ko'rsatiladi, lekin faylga eksport qilinganda
    faqat BITTA — eng so'nggi — sana yetarli).

    (c) Yoniga (agar mavjud bo'lsa) veb-jadvaldagi kabi "#N" — kunlik
    tartib raqami (tarmoq papkasidagi "N. domen (kod)" raqamiga mos) —
    biriktiriladi, shunda faylning o'zidan (dasturga kirmasdan) ham qaysi
    raqamli papka bilan bog'liqligini bilish oson bo'ladi."""
    sana = row['sana'] or ''
    edited = row['edited_at'] if 'edited_at' in row.keys() else None
    date_text = edited[:10] if edited else sana  # faqat "YYYY-MM-DD" qismi
    order_num = row['order_num'] if 'order_num' in row.keys() else None
    if order_num:
        return f"{date_text} #{order_num}" if date_text else f"#{order_num}"
    return date_text
...
def last_folder_path():
    ...
    # DEFAULT holatda "Papkaoch" bo'limida sozlangan UMUMIY (kun.oy.yil) manzil
    # EMAS, balki o'sha manzil ICHIDAGI shu board'ga tegishli ANIQ maxsus
    # pastki papka (masalan t_hisobot uchun "Коммент", jaloba uchun "Жалоба",
    # fishing uchun "Фишинг") ko'rsatilishi kerak — chunki umumiy manzilning
    # o'zi barcha uchta bo'lim uchun bitta ota-papka bo'lib, uni to'g'ridan-
    # to'g'ri skanerlashga yuborsak, tizim uchala bo'limning ham (Жалоба,
    # Коммент, Фишинг) ma'lumotlarini ARALASHTIRIB o'qib qo'yadi — bu XATO
    # (har bir board FAQAT o'z papkasi bilan ishlashi kerak).
    #
    # Agar foydalanuvchi shu oynada boshqa manzilni qo'lda kiritsa — bu FAQAT
    # o'sha safargi ko'rish uchun ishlatiladi, keyingi safar oyna qayta
    # ochilganda yana shu avtomatik aniqlangan manzil ko'rsatiladi.
    board = get_board(request.args)
    base_root = get_setting('papkaoch_root_path', '') or ''
    if base_root and board in ('jaloba', 'fishing', 't_hisobot'):
        subfolder = _find_board_daily_subfolder(base_root, board)
        if subfolder:
            return jsonify({'folder_path': subfolder})
    return jsonify({'folder_path': base_root})
...
def papkaoch_list_entries():
    ...
    (eng so'nggisi birinchi), SAHIFALANGAN holda, ixtiyoriy qidiruv (q) bilan.
    ...
    ketmasligi uchun. "q" — havola (url), teglar yoki papka nomi bo'yicha
    qidiradi (masalan xato kiritilgan eski havolani tezda topib, o'chirish
    uchun)."""
    try:
    ...
    params = []
    if q:
        where = ' WHERE (url LIKE ? OR tags_json LIKE ? OR folder_name LIKE ?)'
        like = f'%{q}%'
        params = [like, like, like]

    with get_db() as conn:
        total = conn.execute(f'SELECT COUNT(*) FROM papkaoch_entries{where}', params).fetchone()[0]
        total_pages = max(1, math.ceil(total / per_page))
        if page > total_pages:
            page = total_pages
        offset = (page - 1) * per_page
        rows = conn.execute(
            f'SELECT * FROM papkaoch_entries{where} ORDER BY id DESC LIMIT ? OFFSET ?',
...
@app.route('/api/papkaoch/entries/delete', methods=['POST'])
def papkaoch_delete_entries():
    """"Oxirgi faoliyat" jadvalidan admin BELGILAGAN (checkbox bilan
    tanlagan) yozuv(lar)ni bazadan o'chiradi.

    NIMA UCHUN KERAK: agar hodim Telegram guruhga XATO havola tashlagan
    bo'lsa va bot shunga mos papka ochgan bo'lsa, keyin hodim buni angab
    papkani QO'LDA (Windows Explorer orqali) o'chirib tashlasa — bazadagi
    "papkaoch_entries" yozuvi (status='created') hamon qolib ketadi.
    check_duplicate() FAQAT bazaga qarab tekshirgani uchun (diskni EMAS),
    keyin xuddi shu tartib raqami/havola bilan TO'G'RI havola yuborilsa
    ham, tizim "bu allaqachon ochilgan" deb papka OCHMAY qo'yadi — garchi
    haqiqiy papka diskda endi mavjud bo'lmasa ham. Shu yerdan xato
    yozuv(lar)ni o'chirib tashlash orqali bu holat tuzatiladi — shundan
    keyin to'g'ri havola qayta yuborilganda, tizim buni YANGI havola deb
    hisoblab, normal tartibda papka ochadi (yoki agar papka diskda
    haqiqatan ham hali mavjud bo'lsa — "Papka havolaga moslandi" deb
    xabar beradi, pastdagi process_single_link'ga qarang)."""
    data = request.get_json(silent=True) or {}
    ids = data.get('ids') or []
    try:
        ids = [int(x) for x in ids]
    except (TypeError, ValueError):
        return jsonify({'error': "ID'lar noto'g'ri formatda"}), 400
    if not ids:
        return jsonify({'error': "Hech narsa belgilanmagan"}), 400

    placeholders = ','.join('?' * len(ids))
    with get_db() as conn:
        cur = conn.execute(f'DELETE FROM papkaoch_entries WHERE id IN ({placeholders})', ids)
        conn.commit()
...
def _load_papkaoch_url_map():
    """"Papkaoch" bo'limi (Telegram bot) tomonidan avvalroq ochilgan barcha
    ("created" statusdagi) papkalarning {normalizatsiya qilingan to'liq
    manzil -> havola (URL)} lug'atini qaytaradi.

    NIMA UCHUN KERAK: bot Telegram guruhdagi xabardan havolani o'qib,
    papkani serverda ochganda, o'sha havolani ALLAQACHON papkaoch_entries
    jadvaliga yozib qo'yadi. Keyinroq xodim "Papkadan import qilish"
    (Jaloba/Fishing/T.hisobot statistikasini hisoblash) bosqichida xuddi
    o'sha papkalarni skanerlaganda — havolani QAYTA qo'lda kiritishiga
    hojat yo'q, chunki u allaqachon ma'lum. Shu lug'at orqali papka manzili
    bo'yicha mos havola avtomatik topib, "Link (URL)" maydoniga oldindan
    to'ldirib qo'yiladi (xodim faqat tekshirib tasdiqlaydi)."""
    url_map = {}
    with get_db() as conn:
        rows = conn.execute(
            "SELECT full_path, url FROM papkaoch_entries WHERE status='created' ORDER BY id ASC"
        ).fetchall()
    for r in rows:
        fp = r['full_path']
        if not fp:
        ...
        # ID bo'yicha o'sish tartibida o'qiganimiz uchun, bir xil manzil uchun
        # bir nechta yozuv bo'lsa (odatda bo'lmasligi kerak) — ENG SO'NGGISI
        # (eng yangi yozilgan) g'olib chiqadi.
        url_map[key] = r['url']
    return url_map

def _papkaoch_url_for(url_map, container_path, folder_name):
    """Berilgan disk manzili (container_path/folder_name) uchun mos
    papkaoch havolasini (agar bot shu manzilda papka ochgan bo'lsa)
    qaytaradi — topilmasa None."""
    if not folder_name:
        return None
    candidate = os.path.join(container_path, folder_name)
...
def _build_jaloba_fishing_section(board, folder_path, sana):
    ...
    url_map = _load_papkaoch_url_map()
    ...
    for order_num in all_orders:
        ...
        # "links" jadvalida hali yozuv yo'q (yangi papka) bo'lsa — havolani
        # "Papkaoch" bo'limi (bot) allaqachon bilishi mumkin, shu yerdan
        # avtomatik topib olamiz (xodim qayta qo'lda kiritmasin deb).
        papkaoch_url = None
        if not link and fc:
            papkaoch_url = _papkaoch_url_for(url_map, folder_path, fc['folder_name'])
        ...
        rows_out.append({
            ...
            'url': link['url'] if link else (papkaoch_url or None),
            'url_from_papkaoch': bool(not link and papkaoch_url),
...
def _build_t_hisobot_sections(folder_path, sana):
    ...
    for status, container_path in containers:
        ...
        for order_num in all_orders:
            ...
            papkaoch_url = None
            if not link and fc:
                papkaoch_url = _papkaoch_url_for(url_map, container_path, fc['folder_name'])

            # MUHIM: papka nomidagi belgi ("(fb)"/"(ins)"/...) — bu papka
            # "Papkaoch" bo'limi orqali AYNAN o'sha havoladan (masalan
            # instagram.com) ochilganini bildiradi. Shuning uchun bu yerdagi
            # "Link (URL)" ham xuddi o'sha (yoki kamida bir xil ijtimoiy
            # tarmoqqa tegishli) havola bo'lishi kerak — xodim keyinchalik
            # havolani QO'LDA xato platformaga (masalan Instagram papkasiga
            # Facebook havolasini) almashtirib qo'yishi mumkin, shuni
            # ushlab qolish uchun bu yerda ham (Jaloba/Fishing kabi)
            # mos-kelmaslik tekshiruvi qo'llaniladi.
            effective_url = (link['url'] if link else None) or papkaoch_url
            folder_category = guess_folder_category(fc['folder_name']) if fc else None
            link_category = guess_url_category(effective_url) if effective_url else None
            category_mismatch = bool(
                folder_category and link_category and folder_category != link_category
                and status_hint != 'realigned'
            )
...
def preview_screenshot_folder():
    ...
    # MUHIM ("o'zini-o'zi tuzatish" himoyasi): agar berilgan (yoki avtomatik
    # olingan) papka manzili to'g'ridan-to'g'ri KUTILGAN maxsus papka
    # ("Жалоба"/"Фишинг"/"Коммент") emas, balki masalan UMUMIY (kun.oy.yil)
    # ota-papka bo'lsa — bu holda ICHIDA barcha uchta bo'lim (Жалоба,
    # Коммент, Фишинг) yonma-yon turgan bo'ladi. Bunday manzilni to'g'ridan-
    # to'g'ri skanerlash MUTLAQO NOTO'G'RI natija beradi: masalan t_hisobot
    # o'zining "Коммент" ma'lumotlaridan tashqari, unga aloqasi yo'q
    # "Жалоба" va "Фишинг" papkalaridagi ma'lumotlarni ham ARALASHTIRIB
    # olib qo'yadi. Shu sabab bu yerda: agar berilgan papka o'zi kutilgan
    # nomga mos kelmasa, ICHIDAN board'ga mos maxsus pastki papkani avtomatik
    # qidirib topamiz va shuni ishlatamiz (topilmasa — quyidagi ogohlantirish
    # bilan, berilgan manzilning o'zi ishlatiladi, chunki bu holat kamdan-kam
    # nostandart, lekin haqiqiy maqsad papka bo'lishi ham mumkin).
    _expected_now = COMMENT_FOLDER_NAMES if board == 't_hisobot' else BOARD_SPECIAL_FOLDER_NAMES.get(board, ())
    if _expected_now and os.path.basename(os.path.normpath(folder_path)) not in _expected_now:
        _auto_sub = _find_board_daily_subfolder(folder_path, board)
        if _auto_sub:
            folder_path = _auto_sub
    ...
    with get_db() as conn:
        for u in updates:
            ...
            if row is None:
                ...
                #
                # MUHIM (T.hisobot uchun): bitta havola BIR NECHTA XIL rukunda
                # (masalan "Umumiy" VA "Tashrif") alohida-alohida papka bo'lib
                # ochilgan bo'lishi TABIIY holat (masalan "Papkaoch" bo'limida
                # "boshqa papkada ham ochilsinmi?" so'roviga "HA" deb javob
                # berilgan bo'lsa). Shu sabab bu yerda "status" ham albatta
                # solishtiriladi — aks holda ikkinchi rukunni skanerlab
                # tasdiqlaganda, birinchi rukunning yozuvi "o'g'irlanib"
                # (uning status'i almashtirilib) ketadi, natijada har ikkala
                # rukun ham navbatma-navbat "Yangi" bo'lib ko'rinaverar edi.
                if is_th:
                    row = conn.execute(
                        'SELECT * FROM links WHERE board=? AND url=? AND status=?',
                        (board, url, row_status)
                    ).fetchone()
                else:
...
def get_links():
    ...
    try:
        ...
        per_page = 50
    # per_page=0 -> "Barchasi" (cheklovsiz, hammasi bitta "sahifada" ko'rsatiladi)
    if per_page not in (50, 100, 500, 1000, 0):
        per_page = 50
    if page < 1:
        page = 1
    ...
    with get_db() as conn:
        ...
        if per_page == 0:
            # "Barchasi" — sahifalashsiz, mos keluvchi BARCHA yozuvlar bitta
            # so'rovda qaytariladi.
            page = 1
            total_pages = 1
            query = (
...
def get_links_grouped():
    """Joriy filtrlarga mos yozuvlarni status bo'yicha guruhlab qaytaradi —
    T.hisobot sahifasidagi "har bir status uchun alohida jadval" ko'rinishini
    chizish uchun ishlatiladi.

    MUHIM: T.hisobot'da haqiqiy "sahifa 1/2/3" navigatsiyasi yo'q (yozuvlar
    statuslar bo'yicha bir nechta alohida jadvalga bo'linib ko'rsatiladi,
    ularni "sahifalarga" bo'lish tabiiy emas). Shu sabab bu yerdagi
    "Sahifada: N ta" tanlovi — sahifalash emas, balki YUKLANADIGAN yozuvlar
    sonini cheklovchi oddiy "eng so'nggi N ta" filtri sifatida ishlaydi
    (ko'rsatilgan filtrlarga mos ENG SO'NGGI faoliyatli N ta yozuv). "Barchasi"
    tanlansa (limit=0) — cheklovsiz, mos keluvchi HAMMA yozuv yuklanadi."""
    board = get_board(request.args)
    clause, params = build_filters(request.args)
    ...
    try:
        ...
        limit = 0
    if limit not in (50, 100, 500, 1000, 0):
        limit = 0
    ...
    return jsonify({'groups': groups, 'total': total, 'shown': len(rows)})
...
def export_excel():
    board = get_board(request.args)
    # MUHIM: agar admin veb-jadvalda ma'lum qatorlarni BELGILAB (checkbox
    # bilan) tanlagan bo'lsa — faqat O'SHA qatorlar eksport qilinadi (joriy
    # filtrlardan qat'i nazar). Aks holda — odatdagidek joriy filtrlarga mos
    # barcha qatorlar eksport qilinadi (avvalgi xatti-harakat saqlanadi).
    ...
    if ids_param:
        ...
        if ids:
            placeholders = ','.join('?' * len(ids))
            query = f'SELECT * FROM links WHERE board=? AND id IN ({placeholders}) ORDER BY id'
            params = [board] + ids
        else:
            query, params = 'SELECT * FROM links WHERE 1=0', []
    else:
        clause, params = build_filters(request.args)
    ...
    if board == 't_hisobot':
        headers = ['#', 'Link', 'Sana', 'Tartib raqami', 'Umumiy izohlar soni', 'Yozilgan izohlar soni', 'Status']
    else:
        headers = ['#', 'Link', 'Sana', 'Tartib raqami', 'Likelar', 'Status']
    ...
    for i, row in enumerate(rows, 1):
        ...
        # MUHIM: "Tartib raqami" endi ALOHIDA (mashinada o'qiladigan) ustunda
        # ham chiqariladi — faqat "Sana" katakchasi ichidagi "#N" matni sifatida
        # EMAS. Aks holda bu faylni QAYTAdan import qilganda ("/api/import")
        # tartib raqami tiklanmay, ketma-ket (1, 2, 3...) qayta raqamlanib
        # ketardi — asl (Papkadan skanerlashda ishlatiladigan) raqam yo'qolib,
        # keyingi skanerlashlarda ko'plab "Nomuvofiqlik"/"Papka topilmadi"
        # xatoliklariga olib kelardi.
        ws.cell(row=i+1, column=4, value=row['order_num'])

        if board == 't_hisobot':
            ws.cell(row=i+1, column=5, value=prev_total_text(row['umumiy_izohlar'], row['prev_umumiy_izohlar']))
            ws.cell(row=i+1, column=6, value=prev_total_text(row['ajratilgan_izohlar'], row['prev_ajratilgan_izohlar']))
            ws.cell(row=i+1, column=7, value=row['status'])
            ...
            ws.cell(row=i+1, column=5, value=prev_total_text(row['likes'], row['prev_likes']))
...
def export_docx():
    board = get_board(request.args)
    # MUHIM: xuddi Excel eksportidagi kabi — admin belgilagan (checkbox bilan
    # tanlagan) qatorlar bo'lsa, faqat o'shalar eksport qilinadi.
    ids_param = (request.args.get('ids') or '').strip()
    if ids_param:
        try:
            ids = [int(x) for x in ids_param.split(',') if x.strip()]
        ...
        if ids:
            placeholders = ','.join('?' * len(ids))
            # Eng oxirgi kiritilgan/tahrirlangan yozuv jadvalning ENG OXIRIDA
            # tursin — quyidagi izohga qarang.
            query = (
                f'SELECT * FROM links WHERE board=? AND id IN ({placeholders}) '
                'ORDER BY COALESCE(edited_at, created_at) ASC, id ASC'
            )
            params = [board] + ids
        else:
            query, params = 'SELECT * FROM links WHERE 1=0', []
    ...
    # Sahifa yo'nalishi: T.hisobot uchun ALBOM (landscape) — chunki jadvalda
    # "Status" ustuni olib tashlanib, "Dastlabki" skrinshot ustuni
    # kattalashtirilgani uchun kengroq joy kerak. Jaloba/Fishing uchun
    # avvalgidek KITOBIY (portret) qoladi.
    section = doc.sections[0]
    if is_thisobot:
        section.orientation = WD_ORIENT.LANDSCAPE
        # python-docx orientatsiyani o'zgartirganda sahifa kengligi/balandligini
        # AVTOMATIK almashtirmaydi — buni qo'lda qilish kerak, aks holda
        # albom deb belgilangan bo'lsa-da sahifa haligacha tor (portret o'lchamda)
        # bo'lib qoladi.
    ...
    def add_screenshot_cell(cell, img_rel, width=None):
        p = cell.paragraphs[0]
        ...
        if img_full and os.path.exists(img_full):
            try:
                run = p.add_run()
                run.add_picture(img_full, width=width or Inches(1.0))
                return
            except Exception:
                pass
    ...
    def add_thisobot_row(table, col_widths, idx, row):
        ...
        # MUHIM: "Status" ustuni olib tashlangani uchun bo'shagan joy shu
        # skrinshot ustuniga qo'shildi — rasm avvalgidan (1.0 inch) kattaroq
        # (1.6 inch) ko'rsatiladi.
        add_screenshot_cell(cells[2], row['screenshot_before'], width=Inches(1.6))
    ...
    if is_thisobot:
        ...
        headers = ['№', 'Link', 'Dastlabki', 'Sana', 'Umumiy izohlar soni', 'Yozilgan izohlar soni']
        col_widths = [Inches(0.4), Inches(2.6), Inches(2.0), Inches(1.1), Inches(1.3), Inches(1.3)]
...
def import_excel():
    ...
    # MUHIM: eksport qilingan fayldagi haqiqiy ustun sarlavhasi "Yozilgan
    # izohlar soni" (bu "ajratilgan_izohlar" bazasi ustuniga mos keladi) —
    # shuning uchun "yozilgan" so'zi ham albatta izlanishi kerak, aks holda
    # bu ustun topilmay, qiymat har doim 0'ga tushib qolardi.
    ajratilgan_col = find_col(['yozilgan izohlar soni', 'yozilgan izohlar', 'yozilgan', 'ajratilgan izohlar soni', 'ajratilgan izohlar', 'ajratilgan'])
    ...
    def parse_date(val):
        ...
        s = str(val).strip()
        # MUHIM: eksport qilingan "Sana" katakchasida "2026-07-15 #3" kabi
        # oxirida tartib raqami belgisi ("#N") bo'lishi mumkin (veb-
        # sahifadagi jadvaldagi kabi qulaylik uchun qo'shilgan). Agar shu
        # faylning o'zi QAYTA import qilinsa, bu qo'shimcha matn sanani
        # tanib bo'lmay qolishiga (va noto'g'ri ravishda BUGUNGI sanaga
        # tushib qolishiga) sabab bo'lardi — shu sabab bu yerda avval
        # o'sha "#N" qismini kesib tashlaymiz.
        s = _SANA_ORDER_SUFFIX_RE.sub('', s).strip()
    ...
    def extract_order_from_sana(val):
        """Agar faylda alohida "Tartib raqami" ustuni bo'lmasa (masalan eski,
        "#N" ustunsiz eksport qilingan fayl) — tartib raqamini "Sana"
        katakchasi ichidagi "#N" matnidan tiklashga harakat qilamiz."""
        if val is None:
            return None
        s = str(val).strip()
        m = _SANA_ORDER_SUFFIX_RE.search(s)
    ...
    def parse_count(val):
        if val in (None, ''):
            return 0
        s = str(val).strip()
        # MUHIM: T.hisobot eksportida "Umumiy izohlar soni"/"Yozilgan izohlar
        # soni" katakchalari "(50) 10" ko'rinishida bo'lishi mumkin (qavs
        # ichida — oldingi kunlargi jami, tashqarisida — joriy son). Faylni
        # qayta import qilganda faqat TASHQARIDAGI (joriy) sonni olamiz —
        # aks holda butun matnni songa aylantirib bo'lmay, son 0'ga tushib
        # qolardi.
            ...
            try:
                ...
                if order_raw not in (None, ''):
                    try:
                        ...
                        order_num_from_file = None
                if order_num_from_file is None and sana_col is not None and sana_col < len(row):
                    # Alohida "Tartib raqami" ustuni topilmadi — eski (shu
                    # ustunsiz) eksport fayli bo'lishi mumkin. Shunda tartib
                    # raqamini "Sana" katakchasi ichidagi "#N" matnidan
                    # tiklashga urinamiz (butunlay yo'qotib qo'ymaslik uchun).
                    order_num_from_file = extract_order_from_sana(row[sana_col])
