...
# Raqam + (ixtiyoriy) ".", ")" yoki bo'shliq(lar) + URL — bittasi bo'lmasa ham
# ishlaydi: "1. url", "1) url", "1 url", "1   url", "1url" — barchasi mos keladi.
_NUMBERED_LINE_RE = re.compile(r'^\s*(\d+)\s*[.\)]?\s*(\S+)\s*$')
# Qatorda FAQAT raqamning o'zi (ixtiyoriy "." yoki ")" bilan) — havola esa
# KEYINGI qatorda bo'lgan holat uchun ("1" / "1." / "1)" alohida qatorda).
...
def parse_telegram_message(text):
    ...
    ham, yoki raqam alohida qatorda, havola esa KEYINGI qatorda bo'lsa
    ham — barchasi to'g'ri tushiniladi):
        #ota_papka
        #bola_papka
        1. https://...
        2 https://...
    ...
    for i, line in enumerate(raw_lines):
        ...
        # 1) "N url" / "N. url" / "N) url" — raqam va havola BITTA qatorda
        m = _NUMBERED_LINE_RE.match(line)
        if m:
            candidate = m.group(2)
            url_match = _URL_RE.match(candidate)
            if url_match:
                links.append((int(m.group(1)), url_match.group(0)))
                continue
            # Raqam bilan boshlangan, lekin undan keyingisi havola emas
            # (masalan "1. Izoh matni") — botga aloqasi yo'q, e'tiborsiz
            # qoldiriladi (xatolik ham hisoblanmaydi).
            continue

        # 2) Shu qatorning o'zida havola bor, lekin oldida raqam yo'q —
        # oldingi (bo'sh bo'lmagan) qatorga qaraymiz: u yolg'iz raqammi?
